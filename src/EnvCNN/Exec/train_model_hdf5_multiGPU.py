...
import EnvCNN.Data.model_checkpoint_parallel as cbk
import time
...
model = models.vgg()
model.summary()
parallel_model = keras.utils.multi_gpu_model(model, gpus=num_of_gpus)
parallel_model.compile(loss="binary_crossentropy", metrics=['accuracy'], optimizer=keras.optimizers.Adam(lr=1e-05))
early_stopping = keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=30, verbose=1, mode='min')
checkpoint = cbk.ModelCheckpoint(os.path.join(output_dir, "model.h5"), monitor='val_loss', verbose=1, save_best_only=True, mode='min', ser_model=model) #Save Model Checkpoint
history = parallel_model.fit_generator(train_gen, steps_per_epoch=math.ceil(num_train_samples / batch_size), validation_data=val_gen, validation_steps=math.ceil(num_val_samples / batch_size), epochs=200, verbose=2, class_weight=class_weights, callbacks=[checkpoint, early_stopping])
hdf5_file.close()
